...
function addGroup(g) { queue.value.push({ name: g, section: '手动' }) }
...
async function aiArrange() {
  ...
  try {
    const profiles = await Promise.all(groups.value.map(async name => {
      ...
      return { name, energy, joint: joints.indexOf(Math.max(...joints)) }
    }))
    ...
    let last = null
    const pick = (band, section) => {
      const pool = bands[band] || bands.mid
      const choice = pool.find(x => x.name !== last?.name && x.joint !== last?.joint) || pool.find(x => x.name !== last?.name) || pool[0]
      last = choice
      return { name: choice.name, section }
    }
    // Anti-Hero 的结构感：轻前奏 → 交替主歌 → 高能副歌 → 回落间奏 → 最终副歌。
    queue.value = [pick('low', '前奏'), pick('mid', '前奏'), pick('mid', '主歌'), pick('low', '主歌'), pick('mid', '主歌'), pick('high', '副歌'), pick('mid', '副歌'), pick('high', '副歌'), pick('low', '间奏'), pick('mid', '间奏'), pick('high', '最终副歌'), pick('mid', '最终副歌'), pick('high', '最终副歌')]
    message.success(`AI 已生成 ${queue.value.length} 步分段编舞：前奏、主歌、副歌与间奏各有不同能量`)
