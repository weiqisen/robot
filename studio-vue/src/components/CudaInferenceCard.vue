...
<template>
  <a-card size="small" class="cuda-card" :body-style="{ padding: '8px 10px' }">
    <div class="cuda-head">
      <span>CUDA 推理监控</span>
      <div class="gpu-spark"><small>GPU {{ state.jetson?.gpu ?? '—' }}%</small><MiniChart :data="gpuHistory" :min="0" :max="100" :height="24" :axis="false" color="#8b5cf6" /></div>
...
.cuda-head { display:flex; align-items:center; gap:7px; color:var(--text-3); font-size:11px; }
.cuda-head :deep(.ant-tag) { margin:0; font-size:10px; line-height:18px; }
.gpu-spark { display:flex; align-items:center; gap:5px; flex:1; min-width:78px; max-width:120px; margin-left:auto; }
.gpu-spark small { color:var(--text-4); white-space:nowrap; font:10px var(--font-code); }
.gpu-spark :deep(svg) { flex:1; min-width:38px; }
