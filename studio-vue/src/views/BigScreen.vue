...
function playActionRow(row, ms) {
  ...
  try { row.servos.forEach((pulse, k) => twinRef.value?.animateJointByServoId([1, 2, 3, 4, 5, 10][k], pulse, ms)) }
...
async function runActionGroup() {
  ...
  try { for (let i=0; i<j.rows.length && actionRunning.value; i++) { const row=j.rows[i], ms=row.time||1000; actionStep.value=i+1; playActionRow(row, ms); await new Promise(res=>setTimeout(res,ms)) } } finally { actionRunning.value=false }
}
...
.scene-action-card{position:absolute;z-index:4;right:15px;bottom:142px;width:300px;padding:9px 10px;border:1px solid rgba(56,189,248,.28);border-radius:9px;background:rgba(8,12,18,.84);backdrop-filter:blur(8px);color:#E2E8F0}.scene-action-card>div:first-child{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px}.scene-action-card small{color:#64748B;font-size:9px}.scene-action-card b{font-size:10px;color:#7DD3FC}.scene-action-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:4px}.scene-action-grid button{min-width:0;height:31px;padding:2px 3px;border:1px solid rgba(148,163,184,.22);border-radius:4px;background:rgba(255,255,255,.04);color:#CBD5E1;font-size:9px;cursor:pointer;display:flex;align-items:center;gap:2px}.scene-action-grid button span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;text-align:left}.scene-action-grid button i{font-style:normal;color:#38BDF8;font-size:8px}.scene-action-grid button.selected{border-color:#F59E0B;background:rgba(245,158,11,.16)}.scene-action-grid button.running{border-color:#F59E0B;background:#B45309;color:#FFF7ED;box-shadow:0 0 9px rgba(245,158,11,.45)}.scene-action-buttons{display:flex;gap:6px;margin-top:6px}.scene-action-buttons button{flex:1;padding:4px;border:1px solid rgba(56,189,248,.4);border-radius:5px;background:rgba(14,116,144,.25);color:#7DD3FC;font-size:11px;cursor:pointer}.scene-action-buttons button:disabled{opacity:.4;cursor:default}.scene-action-buttons button.stop{border-color:rgba(251,113,133,.5);background:rgba(127,29,29,.3);color:#FB7185}
