...
<template>
  <a-alert type="info" show-icon style="margin-bottom:16px"
    message="这是机器人上的第二块板子：幻尔 ros_robot_controller 扩展板（STM32），Jetson 通过串口跟它通信"
    description="电池采样、板载 IMU、按键、手柄/航模接收机、蜂鸣器/LED/OLED、总线舵机与电机驱动都挂在它上面。注意：总线舵机的电压和温度读不到 —— 唯一的接口 bus_servo/get_state 有厂商 bug（调用不存在的 Board.bus_servo_read_voltage），一调用整个节点就崩，所以这里不提供。" />
