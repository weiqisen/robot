...
当任务只需要视觉抓取或远程监控时，可在 **运维面板** 打开“自主导航待机”。它会停止
`explorer-agent` 与在线 SLAM/Nav2 栈，释放这部分 CPU/内存；相机、视觉抓取、雷达守护和
`nav-safety` 仍继续运行。待机期间，探索、返航和点选导航都不可用。

需要移动前关闭待机，等待 `/map`、TF、雷达和 Nav2 均恢复就绪后再解锁。部署会重新安装并启用
导航服务，因此完成部署后若仍不需要导航，请再次切换到待机。
...
- 近障硬急停持续达到阈值时，探索 agent 会扩大当前 Frontier 的临时黑名单，并请求 Nav2 向车尾
 规划一次短距离脱困/转向，再重新评估边界；它不会直接发布 `/cmd_vel`。脱困失败或次数耗尽时仍会
 继续评估其他 Frontier，不会因此错误宣告“无路”。
- 相机深度与 YOLO 都会参与前向禁行区判断。识别到床、桌、椅、沙发等且落在车体前方保护盒时，
 由 `nav-safety` 输出零速度；视觉不确定时宁可停下，不以视觉绕过雷达安全闸门。
...
- 雷达离线时，网页可经二次确认进入“无雷达降级驾驶”：只授权 `manual` 控制源 60 秒，
  前进/横移硬限速 `0.05m/s`、旋转硬限速 `0.20rad/s`，速度指令超过 300ms 未刷新即停车，
  到期自动锁定。该模式不向 Nav2、探索或自动抓取开放；雷达恢复后自动切回正常手动安全模式。
