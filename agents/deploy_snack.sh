...
echo "== 安装 systemd 服务"
$SSH "$USER_@$ROBOT" "sudo tee /etc/systemd/system/start_app_node.service >/dev/null <<'EOF'
[Unit]
Description=JetRover minimal web-workbench ROS bringup
...
User=$USER_
WorkingDirectory=/home/$USER_
ExecStart=/usr/bin/zsh -c 'source /home/$USER_/.zshrc; exec ros2 launch /home/$USER_/web_bringup.launch.py'
Restart=always
RestartSec=10
KillMode=mixed
...
$SSH "$USER_@$ROBOT" "sudo tee /etc/systemd/system/vision-video.service >/dev/null <<'EOF'
[Unit]
Description=JetRover isolated visual grasp MJPEG bridge
After=snack-butler.service network-online.target
Requires=snack-butler.service

[Service]
User=$USER_
WorkingDirectory=/home/$USER_
ExecStart=/usr/bin/zsh -c 'source /home/$USER_/.zshrc; exec python3 /home/$USER_/vision_stream_server.py'
Restart=always
RestartSec=2
...
Cmnd_Alias JETROVER_WEBCTL_RESTART = /usr/bin/systemctl restart start_app_node.service, /usr/bin/systemctl restart webctl.service, /usr/bin/systemctl restart jetson-agent.service, /usr/bin/systemctl restart snack-butler.service, /usr/bin/systemctl restart explorer-agent.service, /usr/bin/systemctl restart exploration-nav.service, /usr/bin/systemctl restart nav-safety.service, /usr/bin/systemctl restart lidar-watchdog.service, /usr/bin/systemctl restart webrtc-agent.service, /usr/bin/systemctl restart llm-agent.service, /usr/bin/systemctl stop start_app_node.service, /usr/bin/systemctl stop jetson-agent.service, /usr/bin/systemctl stop snack-butler.service, /usr/bin/systemctl stop explorer-agent.service, /usr/bin/systemctl stop exploration-nav.service, /usr/bin/systemctl stop nav-safety.service, /usr/bin/systemctl stop lidar-watchdog.service, /usr/bin/systemctl stop webrtc-agent.service, /usr/bin/systemctl stop llm-agent.service, /home/ubuntu/enable_desktop.sh, /home/ubuntu/disable_desktop.sh, /home/ubuntu/pause_navigation_stack.sh, /home/ubuntu/resume_navigation_stack.sh
