...
$SSH "$USER_@$ROBOT" 'sudo systemctl daemon-reload && sudo systemctl enable snack-butler lidar-watchdog nav-safety exploration-nav explorer-agent && sudo systemctl restart snack-butler lidar-watchdog nav-safety exploration-nav explorer-agent'
echo "== snack-butler / lidar-watchdog / nav-safety / exploration-nav / explorer-agent 已启动"
