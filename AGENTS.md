...
- 前端位于 `robot-dashboard-front/`，技术栈是 Vue 3 + Vite + Ant Design Vue；没有 vue-router，页面由 `App.vue` 的 hash 菜单切换。
...
- 正式部署入口是 `agents/deploy_snack.sh`；它会构建 `robot-dashboard-front/dist`，不要手工编辑 `dist`。
...
- 前端对机器人地址的统一来源是 `robot-dashboard-front/src/composables/useRos.js`。新增页面通常还要在 `robot-dashboard-front/src/App.vue` 注册菜单项。
